...
if (isset($_POST['depSelOpt'])){
		...
		if ($_SESSION['ctrl'] == 1){
			$query_search = "SELECT `c`.`id`,`c`.`descr`,`c`.`performed`,`c`.`answer`,`c`.`dep_id`,`c`.`spec_id`,`c`.`date`,`c`.`ctrl`,`c`.`comment`,`d`.`short` AS `dep_name`, `n`.`name` AS `spec_name`,`i`.`descr` AS `item_descr` FROM `control` AS `c` LEFT JOIN `department` AS `d` ON (`c`.`dep_id`=`d`.`id`) LEFT JOIN `name` AS `n` ON (`c`.`spec_id`=`n`.`id`) LEFT JOIN `control_item` AS `i` ON (`c`.`control_id`=`i`.`id`) WHERE `c`.`dep_id`='$search' AND `c`.`ctrl`=1 ORDER BY `c`.`date`";
		}
		else{
...
//		`d`.`short` AS `dep_name`, 
//		`n`.`name` AS `spec_name`,
//		`i`.`descr` AS `item_descr` FROM `control` AS `c` 
//		LEFT JOIN `department` AS `d` ON (`c`.`dep_id`=`d`.`id`) 
//		LEFT JOIN `name` AS `n` ON (`c`.`spec_id`=`n`.`id`) 
//		LEFT JOIN `control_item` AS `i` ON (`c`.`control_id`=`i`.`id`) 
//		WHERE `c`.`dep_id`='$search' AND 
//		((`c`.`ctrl`=0 AND `c`.`date`='$current_date' )
//			OR (`c`.`ctrl`=0 AND
//				'$day' > CONVERT(SUBSTRING_INDEX(`c`.`date`,'-',1),UNSIGNED INTEGER) 
//				AND '$month' >= CONVERT(SUBSTRING(`c`.`date`,4,2),UNSIGNED INTEGER)
//				AND '$year' >= CONVERT(SUBSTRING(`c`.`date`,7,4),UNSIGNED INTEGER))
//			OR (`c`.`ctrl`=0 AND 
//				'$month' > CONVERT(SUBSTRING(`c`.`date`,4,2),UNSIGNED INTEGER)
//				AND '$year' >= CONVERT(SUBSTRING(`c`.`date`,7,4),UNSIGNED INTEGER))
//		)" ;
